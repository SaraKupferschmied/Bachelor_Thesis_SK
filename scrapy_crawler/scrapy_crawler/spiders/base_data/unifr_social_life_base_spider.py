...
class UnifrSocialLifeBaseSpider(scrapy.Spider):
    """Reusable base spider for UNIFR social-life information pages."""
    ...
    TOPIC = "social_life"
    SOURCE_TYPE = "unifr_social_life_page"
    DOC_KEY_PREFIX = "unifr_social_life"
    RECORD_TYPE = "social_life_page"
    MAX_RECURSIVE_DEPTH = 0
    RELEVANT_LINK_KEYWORDS: set[str] = set()
    EXCLUDED_LINK_KEYWORDS: set[str] = set()
    ...
    def parse_page(self, response: scrapy.http.Response):
        ...
        yield {
            ...
            "record_type": self.RECORD_TYPE if depth == 0 else f"linked_{self.RECORD_TYPE}",
    ...
    def _is_relevant_recursive_link(self, link: dict[str, str]) -> bool:
        ...
        if any(keyword in text for keyword in self.EXCLUDED_LINK_KEYWORDS):
            return False
        parsed = urlparse(link.get("url", ""))
        if self.ALLOWED_PATH_PREFIXES and not parsed.path.startswith(self.ALLOWED_PATH_PREFIXES):
            return False
    ...
    def _is_parseable_html_url(self, url: str) -> bool:
        ...
        if not parsed.netloc.endswith("unifr.ch"):
            return False
        low = url.lower()
        if any(low.endswith(ext) or f"{ext}?" in low for ext in (".pdf", ".doc", ".docx", ".xls", ".xlsx", ".zip", ".jpg", ".jpeg", ".png")):
            return False
    ...
    def _classify_link(self, label: str, url: str) -> str:
        ...
        if ".pdf" in s:
            return "pdf"
        if any(x in s for x in ("contact", "office", "service", "support", "help", "beratung")):
            return "support_or_contact"
        if any(x in s for x in ("sport", "culture", "association", "student", "activity", "event")):
            return "social_or_activity"
        if any(x in s for x in ("housing", "accommodation", "living", "restaurant", "library", "infrastructure")):
            return "campus_life_resource"
        return "reference"
    ...
    def _is_non_content_link(self, url: str, label: str) -> bool:
        ...
        if "my.unifr.ch" in low_url or "outlook.com" in low_url:
            return True
        if any(skip in low_url for skip in ("/map", "agenda", "calendar", "#")):
            return True
        return low_label in {"back", "quick links", "schnellzugriff", "fr", "de", "en", "map & orientation", "maps/orientation", "agenda"}
    ...
    def _looks_like_navigation_heading(self, heading: str) -> bool:
        return heading.strip().lower() in {
            ...
            "campus", "support", "infrastructures", "activities", "choose fribourg",
        }
