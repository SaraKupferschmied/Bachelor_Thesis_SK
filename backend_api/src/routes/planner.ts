...
const DAY_TIME_RE = /(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday|Montag|Dienstag|Mittwoch|Donnerstag|Freitag|Samstag|Sonntag)\s+(\d{1,2}:\d{2})\s*-\s*(\d{1,2}:\d{2})/gi;
...
function groupTimeBlocks(group: PlanGroup) {
  const blocks: Array<{ day: string; start: number; end: number; code: string }> = [];
  for (const option of group.options) {
    const text = option.day_time_info ?? "";
    for (const match of text.matchAll(DAY_TIME_RE)) {
      blocks.push({
        day: match[1]!.toLowerCase(),
        start: minutes(match[2]!),
        end: minutes(match[3]!),
        code: option.code,
      });
    }
...
function groupsConflict(a: PlanGroup, b: PlanGroup): boolean {
  for (const left of groupTimeBlocks(a)) {
    for (const right of groupTimeBlocks(b)) {
      if (left.code === right.code) continue;
      if (left.day === right.day && left.start < right.end && right.start < left.end) return true;
    }
  }
  return false;
}

function slotHasConflict(slot: PlanSlot, group: PlanGroup): boolean {
  return [...slot.mandatory, ...slot.electives].some((existing) => groupsConflict(existing, group));
}
...
function buildSuggestedPlan(
  ...
  const slots: PlanSlot[] = Array.from({ length: semesters }, (_, i) => ({
    semester_number: i + 1,
  ...
  for (const group of sortedMandatory) {
    ...
    if (!candidates.length) {
      candidates = slots.filter((s) => fitsSemester(group, s.semester_type) && !slotHasConflict(s, group));
    }
...
export async function plannerRoutes(app: FastifyInstance) {
  ...
  app.get(
      ...
      schema: {
        ...
        querystring: {
          ...
          properties: {
            ...
            completed_course_codes: { type: "string", description: "Comma-separated course codes already completed by the student" },
            locale: { type: "string", enum: ["de", "en", "fr"] },
          },
        },
      },
    ...
    async (req, rep) => {
      const { program_id, semesters, locale, total_ects, selected_elective_codes, completed_course_codes } =
      ...
      const rows = await query<ProgramPlanCourseRow>(
        ...
        SELECT
          ROW_NUMBER() OVER (ORDER BY co.ctid) AS row_no,
          co.program_id,
          co.code,
      ...
      const courses = rows
        .map(toPlanCourse)
        .filter((c) => !completedCodeSet.has(normalizeCourseCode(c.code)));
      ...
      return {
        ...
        completed_course_codes: [...completedCodeSet],
